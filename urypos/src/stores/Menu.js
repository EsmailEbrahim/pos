    ...
    actions: {
        ...
        orderTypeSelection() {
            ...
            if (this.cart.length > 0) {
                this.alert
                    .createAlert(
                        "الطلب ليس فارغ",
                        "الرجاء تفريغ محتويات الطلب قبل تحديد نوع الطلب.",
                        "OK"
                    )
                    .then(() => {
        ...
        addToCart(item) {
            ...
            if (!itemIndexExists) {
                ...
                let message = `تم إضافة '${item.item}' إلى الطلب.`;
        ...
        decrementItemQuantity(item) {
            ...
            if (itemIndexExists) {
                ...
                let message =
                    item.qty > 0
                        ? `تم تحديث كمية '${item.item}' في الطلب إلى: ${item.qty}`
                        : `تم حذف '${item.item}' من الطلب.`;
                // this.notification.createNotification(message);
            }
        },
